  ...
  $: pts = range2(k).map((pos, i) => {
    ...
    return {
      ...
      transformCache: ($t) =>
        `translate(${
          (p + pos[0] * (1 - 2 * p)) * w +
          ((pos[1] < 0.5 ? -$t.displaceX : $t.displaceX) * w) / 5
        } ${
          (p + pos[1] * (1 - 2 * p)) * w +
          ((pos[0] < 0.5 ? -$t.displaceY : $t.displaceY) * w) / 5
        })`,
      color: tinycolor("#185E77")
        .darken(12 * colorFactor)
        .toHexString(),
      size,
      absoluteSize,
...
<svg width="100%" height="100%" viewBox="0 0 {w} {w}">
  <g
    transform="translate({w / 2} {w / 2}) rotate({$t.rotation * 4}) translate({-w / 2} {-w / 2})">
    {#each pts as pt}
      <g transform={pt.transformCache($t)}>
        <circle cx={0} cy={0} r={pt.absoluteSize} fill={pt.color} />
      </g>
    {/each}
    {#each pts as pt}
      <g transform={pt.transformCache($t)}>
